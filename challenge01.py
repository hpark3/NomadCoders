...
while True:
    ...
    try:
        menu_num = input("select a number on the menu: ")
        menu = int(menu_num)
    except ValueError:
        print('{}는 숫자가 아닙니다.'.format(menu_num))  # 에러 발생 시 처리할 코드
    ...
    # 여기서는 else 가 적절
        ...
        try:
            if( menu>0 and menu <7):
                a_num = input("1st number: ")
                b_num = input("2nd number: ")
                a= int(a_num)
                b= int(b_num)
            ...
            print('{}와 {}는 자료형이 달라서 계산 불가.'.format(a_num, b_num))
